...
def main():
    try:
        ...
        book_data = Serialization(book, filename="tmp/addressbook.pkl").load_data()
        if book_data:
            book = book_data
        while True:
            ...
            if command in ["close", "exit"]:
                ...
                if len(args) >= 3:
                    ...
                    if record:
                        ...
                        print(
                            f"Changed phone from {old_phone} to {new_phone} for {name}."
                        )
                    else:
        ...
        print(f"Unexpected error has been occurred: {ex}")

    finally:
        Serialization(book, filename="tmp/addressbook.pkl").save_data()
